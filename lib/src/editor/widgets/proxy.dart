...
class CustomTextProxy extends SingleChildRenderObjectWidget {
  const CustomTextProxy(Widget child, {super.key}) : super(child: child);

  @override
  RenderCustomTextProxy createRenderObject(BuildContext context) {
    return RenderCustomTextProxy(null);
  }
}

class RenderCustomTextProxy extends RenderProxyBox
    implements RenderContentProxyBox {
  RenderCustomTextProxy(super.child);

  RenderParagraphProxy? richTextProxyChild;

  @override
  List<TextBox> getBoxesForSelection(TextSelection selection) {
    _searchChild();
    if (richTextProxyChild == null) {
      throw Exception(
          "The customTextLineNodeBuilder need to have on some of it's children the TextLine child");
    }
    return richTextProxyChild!.getBoxesForSelection(selection);
  }

  void _searchChild() {
    final box = child;
    if (richTextProxyChild != null) return;
    box!.visitChildren((renderObject) {
      if (renderObject is RenderParagraphProxy) {
        richTextProxyChild = renderObject;
        return;
      }
    });
  }
  ...
  double? getFullHeightForCaret(TextPosition position) {
    ...
    return richTextProxyChild!.getFullHeightForCaret(position);
  }

  @override
  Offset getOffsetForCaret(TextPosition position, Rect caretPrototype) {
    //TODO: here we will need to search the padding or container widgets that can contains EdgeInsets param
    // to adjust correctly the offset of the caret
    _searchChild();
    if (richTextProxyChild == null) {
      throw Exception(
          "The customTextLineNodeBuilder need to have on some of it's children the TextLine child");
    }
    return richTextProxyChild!.getOffsetForCaret(position, caretPrototype);
  }

  @override
  TextPosition getPositionForOffset(Offset offset) {
    //TODO: here we will need to search the padding or container widgets that can contains EdgeInsets param
    // to adjust correctly the position
    _searchChild();
    if (richTextProxyChild == null) {
      throw Exception(
          "The customTextLineNodeBuilder need to have on some of it's children the TextLine child");
    }
    return richTextProxyChild!.getPositionForOffset(offset);
  }
  ...
  TextRange getWordBoundary(TextPosition position) {
    ...
    return richTextProxyChild!.getWordBoundary(position);
  }

  @override
  double get preferredLineHeight {
    _searchChild();
    return richTextProxyChild!.preferredLineHeight;
  }
}
