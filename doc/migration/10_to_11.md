...
### The `QuillControllerConfig.onClipboardPaste` is not a fallback anymore when couldn't handle the paste operation by default
...
### No longer handle asset images by default in `flutter_quill_extensions`
...
### No longer request editor focus by default after pressing a `QuillSimpleToolbar`'s button
