...
The `QuillToolbar` widget has been removed as it is no longer needed for `QuillSimpleToolbar` or **custom toolbars**.

It was previously required for provider and localization delegate checks. Additionally, the `QuillToolbarConfigurations` class has been removed.

To migrate, ensure to add the [required localization delegate](#-4-required-localization-delegate) in your app widget
and remove the `QuillToolbar`.

```diff
- QuillToolbar(
-  configurations: const QuillToolbarConfigurations(),
-  child: YourToolbarWidget(),
- );
+ YourToolbarWidget();
```

Customizing the buttons (of the `QuillSimpleToolbar`) from the toolbar config in a custom toolbar is no longer a feature. Instead, you can use the constructor of each button widget.
...
- The `QuillControllerConfig.onClipboardPaste` has been updated to allow to override of the default clipboard paste handling instead of only handling the clipboard paste if the default logic didn't paste. See the updated docs comment of [`QuillControllerConfig.onClipboardPaste`](https://github.com/singerdmx/flutter-quill/blob/d9b572b43a07ede5a4265ae6ac3e0bea783aa894/lib/src/controller/quill_controller_config.dart#L9-L38) for an example.
- **flutter_quill_extensions**: Does not handle `AssetImage` anymore by default when loading images, instead use `imageProviderBuilder` to override the default handling. See [Image assets support](https://pub.dev/packages/flutter_quill_extensions#-image-assets).
