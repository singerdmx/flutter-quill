...
* On `7.x.x` and older versions this was the equivalent of `QuillSimpleToolbar` and `QuillSimpleToolbar` didn't exist.
* Starting from `11.0.0`, the `QuillToolbar` is no longer needed and has been removed, and the `QuillSimpleToolbar` work without. It is no longer
