...
The existing code work and compile but the functionality has changed in a non-backward-compatible way:

- The `QuillControllerConfig.onClipboardPaste` has been updated to allow to override the default clipboard paste handling instead of only handling the clipboard paste if the default logic didn't paste. See the updated docs comment of [`QuillControllerConfig.onClipboardPaste`](https://github.com/singerdmx/flutter-quill/blob/eed5629f0b037b1377e4c3693464e8ccd484b1c2/lib/src/controller/quill_controller_config.dart#L38) for an example.
