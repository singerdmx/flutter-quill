...
YourCustomToolbar(
  buttons: [
    // Using buttons of the `QuillSimpleToolbar` in your custom toolbar
...
> [!NOTE]
> This might be confusing: `QuillToolbar` is not a toolbar on its own like `QuillSimpleToolbar`. It's a widget that only 
...
- The `QuillToolbar` widget is utilized within `QuillSimpleToolbar` and can also be used to build a custom toolbar.
...
is only needed if you use the library’s toolbar buttons from `flutter_quill`.
